...
KIND_CLUSTER ?= pod-identity-reloader-test-e2e
...
setup-test-e2e: ## Set up a Kind cluster for e2e tests if it does not exist
	...
	KIND_CLUSTER_NAME=$(KIND_CLUSTER) hack/local-dev/kind-up.sh
...
cleanup-test-e2e: ## Tear down the Kind cluster used for e2e tests
	KIND_CLUSTER_NAME=$(KIND_CLUSTER) hack/local-dev/kind-down.sh
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name pod-identity-reloader-builder
	$(CONTAINER_TOOL) buildx use pod-identity-reloader-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm pod-identity-reloader-builder
	rm Dockerfile.cross
